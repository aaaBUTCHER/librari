...
exports.getBookProfile = async (req, res) => {
    ...
    const libriIFLitruar = libriIDerguar.filter(c => {
        return c.id == id
    });
    const [libriIVetem] = libriIFLitruar;
    console.log(req.session.user.privilegji)
    res.render("librat/profiliILibrit", {
        libri: libriIVetem,
        librrrr: libriIDerguar,
...
exports.deleteABook = async (req, res) => {
    ...
    // await Librat.deletaABook(req.params.id);
    await res.render("dashboard/crudiPerLibra", {
        aaa1: Librat.getAllBooks(),
        aaa2: Librat.getAllSelles(),
        isAuthenticated: req.session.isLoggedIn,
        err: errori
    });
}
...
exports.getAllBooks = async (req, res) => {
    ...
    if (!(Object.keys(zhanri).length === 0) && zhanri.zhanri !== "clear") {
        ...
        filterdBooks = array.filter((c) => {
            return zhanri.zhanri === c.zhanri;
        });
        return res.render("librat/index", {
            titulliIfaqes: "Librat me zhaner: " + zhanri.zhanri,
            active: zhanriAktiv,
            librat: await filterdBooks,
            isAuthenticated: req.session.isLoggedIn,
            privilege: req.session.user.privilegji
        });
    }
    res.render("librat/index", {
        titulliIfaqes: "Te gjitha librat",
        librat: await dbBooks,
        active: zhanriAktiv,
        isAuthenticated: req.session.isLoggedIn,
        privilege: req.session.user.privilegji
    })
...
exports.createBook = async (req, res) => {
    ...
    try {
        ...
        const aaa2 = await Librat.getAllSelles
        const aaa1 = await Librat.getAllBooks();
        await res.render("dashboard/dashboard", {
            aaa1: await Librat.getAllBooks(),
            aaa2: await Librat.getAllSelles(),
            errorActive: "none",
            isAuthenticated: req.session.isLoggedIn
        });
    } catch (err) {
